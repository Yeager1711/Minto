...
import styles from './InviteePopup.module.scss';
...
const InviteePopup: React.FC<InviteePopupProps> = ({ quantity, totalPrice, onClose, id }) => {
    ...
    const exportToExcel = () => {
        const data = inviteeNames.map((name) => ({
            Tên: name,
            'Link Mời': `/template/${id}/${encodeURIComponent(name)}`,
        }));
                            ...
                            <div className={styles.namesList}>
                                {inviteeNames.map((name, index) => (
                                    <div key={index} className={styles.nameItem}>
                                        <span>{name}</span>
                                        <Link
                                            href={`/template/${id}/${encodeURIComponent(name)}`}
...
// const InviteePopup: React.FC<InviteePopupProps> = ({ quantity, totalPrice, onClose, weddingTime, productId }) => {
//     const [isClosing, setIsClosing] = useState(false);
//     const [inviteeNames, setInviteeNames] = useState<string[]>(Array(quantity).fill('')); // Initialize array for names
//     const [showConfirmationPopup, setShowConfirmationPopup] = useState(false);
//     const [confirmationClosing, setConfirmationClosing] = useState(false);
...
//     // Export invitee names to Excel
//     const exportToExcel = () => {
//         const data = inviteeNames.map((name) => ({
//             Tên: name,
//             'Link Mời': `/template/mau_2/${encodeURIComponent(name)}`,
//         }));
...
//                         onClick={() => setConfirmationClosing(true)}
//                     >
//                         <FontAwesomeIcon icon={faXmark} />
//                     </button>
//                     <div
...
//                             <h2 className={styles.popupTitle}>Danh sách người được mời</h2>
//                             <p className={styles.popupSubtitle}>
//                                 Dưới đây là danh sách tên và link mời:
//                             </p>
//                         </div>
//                         <div className={styles.popupBody}>
//                             <div className={styles.namesList}>
...
//                                         <span>{name}</span>
//                                         <Link
//                                             href={`/template/mau_2/${encodeURIComponent(name)}`}
//                                             className={styles.nameLink}
//                                             target="_blank"
//                                             rel="noopener noreferrer"
//                                         >
