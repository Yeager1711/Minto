...
const InviteePopup: React.FC<InviteePopupProps> = ({
    ...
    const [formattedTotalPrice, setFormattedTotalPrice] = useState<string>('0 VND');
    ...
    const calculateTotalPrice = React.useCallback(() => {
        if (templatePrice === null) return 0;

        let totalPrice = Number(templatePrice);

        if (quantity > 10) {
            const extraPeople = quantity - 10;
            totalPrice += extraPeople * priceCardDefault;
        }

        if (isEligibleForDiscount) {
            totalPrice = totalPrice * (1 - DiscountEligibility);
        }

        const now = new Date();
        const startDate = new Date('2025-09-01T00:00:00+07:00');
        const endDate = new Date('2025-09-03T12:00:00+07:00');

        if (now >= startDate && now <= endDate) {
            totalPrice = totalPrice * 0.9; // Giảm thêm 10%
        }

        return totalPrice;
    }, [templatePrice, quantity, isEligibleForDiscount]);

    // ✅ Update giá hiển thị mỗi khi thay đổi
    useEffect(() => {
        const finalPrice = calculateTotalPrice();
        setFormattedTotalPrice(finalPrice.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' }));
    }, [calculateTotalPrice]);
        ...
        >
            <div className={styles.popupHeader}>
                ...
                <p className={styles.popupSubtitle}>
                    ...
                    {isEligibleForDiscount && <span className={styles.discountNote}> (Đã giảm giá)</span>}
                </p>
            </div>
