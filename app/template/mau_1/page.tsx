...
const WeddingInvitation: React.FC = () => {
    ...
    useEffect(() => {
        ...
        coupleImages?.forEach((img, index) => {
            const element = img as HTMLElement; // Type assertion
            element.addEventListener('click', () => {
                coupleImages.forEach((otherImg) => {
                    ...
                    otherElement.setAttribute('style', ''); // Reset order
                });
                element.classList.add(styles.centerImg);
                element.style.order = '0'; // Set order to center the clicked image
                let order = -1;
                coupleImages.forEach((otherImg, otherIndex) => {
                    if (otherImg !== img) {
                        const otherElement = otherImg as HTMLElement;
                        otherElement.style.order = order.toString();
                        order += 2; // Alternate sides: -1, 1
                    }
                });
            });
        });
