...
import { faChartSimple, faEye, faEyeSlash, faRotateLeft, faExpand, faPlus } from '@fortawesome/free-solid-svg-icons';
import styles from './dashboard.module.css';
import AddProduct from 'app/admin/popup/add_template/addTemplates';
import Skeleton from '../Reponsive_Dashboard/Skeleton';
import ErrorList from 'app/admin/error_list/page';
import EditTemplate from 'app/admin/editTemplate/page';
...
import ManageUser from 'app/admin/manage_user/page';
...
const Dashboard: React.FC = () => {
            ...
            default:
                return (
                    <div className={styles.wrap_content}>
                        ...
                        <div className={styles.SpancebetWeen}>
                            <div className={styles.Revenue}>
                                <div className={styles.box_revenue}>
                                ...
                                <div className={styles.flex}>
                                    <div className={styles.box_RealTotalRevenue}>
                                        <span>Total Revenue</span>
                                        <FontAwesomeIcon
                                        ...
                                        <h2>
                                            {isLoading ? (
                                                ...
                                                `${realRevenue.toLocaleString()} VNĐ`
                                            ) : (
                                                '****** VNĐ'
                                            )}
                                        </h2>
                                    ...
                                    <div className={styles.box_TotalVirtualRevenue}>
                                        <span>Total virtual revenue</span>
                                        <FontAwesomeIcon
                            ...
                            <div ref={serverStatusRef} className={styles.server_response}>
                                ...
                                <div className={styles.server_info}>
                                    {serverStatusData.length > 0 ? (
                                        <>
                                            <div className={styles.update_at}>
                                                <span>Update at: </span>
                                                <h3>
                                                    {new Date(
                                            ...
                                            <div className={styles.response_time}>
                                                ...
                                                <h3>{serverStatusData[serverStatusData.length - 1].responseTime} ms</h3>
                                            </div>

                                            <div className={styles.return_time}>
                                                <span>return time: </span>
                                                <h3>{serverStatusData[serverStatusData.length - 1].returnTime} ms</h3>
                                            </div>
                                        </>
                                    ) : (
                        ...
                        {/* <div className={styles.controlCenter} onClick={() => setIsControlCenterOpen(true)}>
                            <FontAwesomeIcon icon={faExpand} className={styles.btn_expand} />
                            {isControlCenterOpen && <ControlCenter onClose={() => setIsControlCenterOpen(false)} />}
                        </div> */}

                        <div className={styles.OrdersByYear}>
                                        ...
                                        <svg width="100" height="100">
                                            ...
                                            <circle
                                                ...
                                                style={{
                                                    strokeDasharray: `${2 * Math.PI * 40}px`,
                                                    strokeDashoffset: calcStrokeOffset(
                                                        apiData.templateUsage.filter((t) => t.usageCount > 0).length,
                                                        apiData.totalTemplates || 1
                                                    ),
                                                }}
                                            />
                                            <text
                                                ...
                                                fill="#333"
                                            >
                                                {`${apiData.templateUsage.filter((t) => t.usageCount > 0).length}/${apiData.totalTemplates || 0}`}
                                            </text>
                                        </svg>
                                    </div>
                                ) : (
                                                ...
                                                .map((template) => (
                                                    <SwiperSlide key={template.templateId}>
                                                        <div className={styles.template_card}>
                                                            <p>Giá: {template.templatePrice.toLocaleString()} VNĐ</p>
    ...
    return (
        <div className={styles.dashboard}>
            <div className={styles.dashboard__bg}></div>
            <div className={styles.dashboard_wrapper}>
                {/* navigation */}
                <div className={styles.navigation_center__dashboard}>
                    <button
                        className={`${styles.nav__item} ${activeSection === 'main' ? styles['nav__item--active'] : ''}`}
                        onClick={() => handleNavChange('main')}
                    >
                        Home
                    </button>
                    <button
                        className={`${styles.nav__item} ${activeSection === 'account' ? styles['nav__item--active'] : ''}`}
                    ...
                    <button
                        title="Thêm sản phẩm"
                        className={`${styles.nav__item} ${styles['nav__item--AddProduct']}`}
                        onClick={openAddProductPopup}
                    >
                        <FontAwesomeIcon icon={faPlus} />
                    </button>
                    <button
                        className={`${styles.nav__item} ${activeSection === 'feedback' ? styles['nav__item--active'] : ''}`}
                        onClick={() => handleNavChange('feedback')}
                    >
                        Feedback
                    </button>
                    <button
                        className={`${styles.nav__item} ${activeSection === 'edit' ? styles['nav__item--active'] : ''}`}
